...
Este repositório contém a implementação completa da análise exploratória do dataset *Wine Quality* do UCI Machine Learning Repository.
...
4. **Análise Bivariada**: Scatter plots por classe para todos os 55 pares de preditores e Matriz de Correlação de Pearson (com heatmap).
5. **Análise Multivariada (PCA)**: Implementação **do zero** (sem `sklearn.decomposition.PCA`) da Análise de Componentes Principais, com padronização dos dados, autodecomposição via `numpy.linalg.eigh` e projeção bidimensional por classe.
...
| **João Otávio**       | Implementação da PCA. Escrita da fundamentação teórica nos métodos do artigo e interpretação dos *loadings* e variância explicada.                                       |
...
**Atenção**: Se sua máquina estiver sem acesso à internet, baixe manualmente os arquivos CSV do UCI e coloque-os na mesma pasta do script:

- `winequality-red.csv`
- `winequality-white.csv`

O script vai gerar uma pasta *outputs* contendo todos os resultados.